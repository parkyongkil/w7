...
public class DatabaseIntrospector {
    ...
    private Map<ActualTableName, List<IntrospectedColumn>> getColumns(
        ...
        while (rs.next()) {
            ...
            try { introspectedColumn.setAutoIncrement("YES".equals(rs.getString("IS_AUTOINCREMENT"))); } //$NON-NLS-1$ //$NON-NLS-2$
            catch (Exception e) { }
            try { introspectedColumn.setGeneratedColumn("YES".equals(rs.getString("IS_GENERATEDCOLUMN"))); } //$NON-NLS-1$ //$NON-NLS-2$
            catch (Exception e) { }
